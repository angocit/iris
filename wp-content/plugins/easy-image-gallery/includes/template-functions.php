        ...
        foreach ($galleries as $gallery){
            ...
            if ( $gallery['SHORTCODE'] == $gallery_id ){
    			...
    			if ( isset($has_gallery_images) && !empty($has_gallery_images) ) {
					?>
	                <div class="easy-image-gallery slider-for <?php echo $classes; ?>">
                    <?php
                    	foreach ( $has_gallery_images as $attachment_id ) {
	                        ...
	                        $image = wp_get_attachment_image( $attachment_id, apply_filters( 'easy_image_gallery_thumbnail_image_size', 'full' ), '', array( 'alt' => trim( strip_tags( get_post_meta( $attachment_id, '_wp_attachment_image_alt', true ) ) ) ) );
	                        ...
	                        if ( isset($gallery['OPEN_IMAGES']) && $gallery['OPEN_IMAGES'] == 'on' )
	                            $html = sprintf( '<div><a %s href="%s" class="%s" title="%s" data-caption="%s" target="_blank">%s</a><div class="gallery-caption">'.$image_caption.'</div></div>', $rel, $image_link, $image_class, $image_caption, $image_caption, $image );
	                        else
					...
					<div class="easy-image-gallery slider-nav <?php echo $classes; ?>">
                    <?php
                    	foreach ( $has_gallery_images as $attachment_id ) {
	                        $classes = array( 'eig-popup' );
