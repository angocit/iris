...
jQuery( function( $ ){
	...
	$("#upsell_project_data").bind("keypress", function(e) {
		if (e.keyCode == 13) return false;
	});

	// Type box
	$('.type_box').appendTo( '#woocommerce-project-data h3.hndle span' );

	$(function(){
		// Prevent inputs in meta box headings opening/closing contents
		$('#woocommerce-project-data h3.hndle').unbind('click.postboxes');

		jQuery('#woocommerce-project-data').on('click', 'h3.hndle', function(event){

			// If the user clicks on some form input inside the h3 the box should not be toggled
			if ( $(event.target).filter('input, option, label, select').length )
				return;

			$('#woocommerce-project-data').toggleClass('closed');
		});
	});
	...
	$( 'select#project-type' ).change( function () {
		...
		$( 'body' ).trigger( 'woocommerce-project-type-change', select_val, $( this ) );

	}).change();

	$( 'body' ).on( 'woocommerce-project-type-change', function( e, select_val ) {
		if ( 'variable' !== select_val && 0 < $( '#variable_project_options input[name^=variable_sku]' ).length && $( 'body' ).triggerHandler( 'woocommerce-display-project-type-alert', select_val ) !== false ) {
			window.alert( woocommerce_admin_meta_boxes.i18n_project_type_alert );
		}
	});
	...
	function show_and_hide_panels() {
		var project_type    = $('select#project-type').val();
		...
		$.each( woocommerce_admin_meta_boxes.project_types, function( index, value ) {
	...
	$('#woocommerce-project-data').on( 'click', '.sale_schedule', function() {
	...
	$('#woocommerce-project-data').on( 'click', '.cancel_sale_schedule', function() {
		var $wrap = $(this).closest( 'div, table' );
	...
	// File inputs
	$('#woocommerce-project-data').on('click','.downloadable_files a.insert',function(){
		$(this).closest('.downloadable_files').find('tbody').append( $(this).data( 'row' ) );
		return false;
	});
	$('#woocommerce-project-data').on('click','.downloadable_files a.delete',function(){
	...
	// Initial order
	var woocommerce_attribute_items = $('.project_attributes').find('.woocommerce_attribute').get();
	...
	$(woocommerce_attribute_items).each( function(idx, itm) { $('.project_attributes').append(itm); } );

	function attribute_row_indexes() {
		$('.project_attributes .woocommerce_attribute').each(function(index, el){
			$('.attribute_position', el).val( parseInt( $(el).index('.project_attributes .woocommerce_attribute') ) );
		});
	};
	...
	$( 'button.add_attribute' ).on('click', function(){
		var size         = $( '.project_attributes .woocommerce_attribute' ).size();
		var attribute    = $( 'select.attribute_taxonomy' ).val();
		var $wrapper     = $( this ).closest( '#project_attributes' ).find( '.project_attributes' );
		var project_type = $( 'select#project-type' ).val();
		var data         = {
	...
	$('.project_attributes').on('blur', 'input.attribute_name', function(){
		$(this).closest('.woocommerce_attribute').find('strong.attribute_name').text( $(this).val() );
	});

	$('.project_attributes').on('click', 'button.select_all_attributes', function(){
		$(this).closest('td').find('select option').attr("selected","selected");
		$(this).closest('td').find('select').change();
		return false;
	});

	$('.project_attributes').on('click', 'button.select_no_attributes', function(){
		$(this).closest('td').find('select option').removeAttr("selected");
		$(this).closest('td').find('select').change();
		return false;
	});

	$('.project_attributes').on('click', 'button.remove_row', function() {
	...
	$('.project_attributes').on('click', 'button.add_new_attribute', function() {

		$('.project_attributes').block({ message: null, overlayCSS: { background: '#fff', opacity: 0.6 } });
	...
	$('.save_attributes').on('click', function(){
		...
		var data = {
			...
			data:			$('.project_attributes').find('input, select, textarea').serialize(),
		...
		$.post( woocommerce_admin_meta_boxes.ajax_url, data, function( response ) {
			...
			$('#variable_project_options').block({ message: null, overlayCSS: { background: '#fff', opacity: 0.6 } });
			$('#variable_project_options').load( this_page + ' #variable_project_options_inner', function() {
				$('#variable_project_options').unblock();
			} );
	...
	jQuery('.add_project_images').on( 'click', 'a', function( event ) {
		...
		// Create the media frame.
		project_gallery_frame = wp.media.frames.project_gallery = wp.media({
			// Set the title of the modal.
			title: $el.data('choose'),
			button: {
				text: $el.data('update'),
